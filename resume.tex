...
\begin{entrylist}
    \entry
    {10/2023 - Present}
    {\href{https://snapp.ir}{Snapp}\newline\small\textnormal{Tehran (on-site)}}
    ...
    \begin{itemize}[itemsep=2pt,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=-1pt]
        ...
        \item {Set up whole infrastructure using \textbf{Redpanda} and \textbf{Flink} resulting in \textbf{80\% perfomance 
        increase}}
        \item{Set up data warehouse using \textbf{Mysql} and stream processing using \textbf{Spark-Streaming}}
    \end{itemize}
    {\vspace{2pt}}
    \texttt{Kafka} \slashsep \texttt{Flink} \slashsep \texttt{S3} \slashsep \texttt{Kubernetes} \slashsep \texttt{Clickhouse} \slashsep \texttt{MySQL}}
\end{entrylist}
% VoxSolutions
\begin{entrylist}
    \entry
